...
// beam half-width. 1.0 matches the legacy GL_POINTS tip length; the
// default is 10% under that, by eye ([vector] endcap_strength).
#define DEF_ENDCAP_STRENGTH  0.9f
...
static float cfg_endcap = DEF_ENDCAP_STRENGTH;
...
int beam_init(void)
{
	...
	cfg_endcap = get_config_float("vector", "endcap_strength", DEF_ENDCAP_STRENGTH);
	...
	set_config_float("vector", "endcap_strength", cfg_endcap);
...
void beam_set_endcap_strength(float s) { cfg_endcap = s; }
...
static void build_joins(void)
{
	...
	while (i < g_nverts)
	{
		...
		r = half * ((count >= 2) ? cfg_corner : cfg_endcap);
